...
test("card effects use the original repeated symbols, never numbered icon overlays", () => {
    const renderer = new CardRenderer();
    const html = renderer.render_symbols({attack:3, defense:2, healing:0, draw:1, play_again:0});
    assert.equal((html.match(/class="card-symbol symbol-/g) ?? []).length, 6);
    assert.equal((html.match(/symbol-attack/g) ?? []).length, 3);
    assert.equal((html.match(/symbol-defense/g) ?? []).length, 2);
    assert.equal((html.match(/symbol-draw/g) ?? []).length, 1);
    assert.doesNotMatch(html, /symbol-value|symbol-glyph|symbol-healing|symbol-play_again/);
    const shield = renderer.render_card_face(get_card_definition('grandpa_screen_saver'));
    assert.equal((shield.match(/symbol-defense/g) ?? []).length, 4, 'Screen Saver shows four separate shields');
    for (const card of get_all_card_definitions()) {
        const symbols = renderer.render_symbols(card.symbols);
        for (const [name, count] of Object.entries(card.symbols)) {
            assert.equal((symbols.match(new RegExp(`symbol-${name}`, 'g')) ?? []).length, count, `${card.name}: ${name}`);
        }
    }
});
