...
try {
    ...
    assert.equal(await action(page, 'toggle-hand').getAttribute('aria-expanded'), 'false', 'human turn does not pin the hand over the last CPU play');
    const beforePeek = await page.evaluate(() => JSON.stringify(testGame.game_state));
    await action(page, 'toggle-hand').hover();
    assert.equal(await action(page, 'toggle-hand').getAttribute('aria-expanded'), 'true');
    await page.mouse.move(2, 2);
    assert.equal(await action(page, 'toggle-hand').getAttribute('aria-expanded'), 'false', 'leaving a peek restores the board before playing');
    await action(page, 'toggle-hand').click();
    await page.mouse.move(2, 2);
    assert.equal(await action(page, 'toggle-hand').getAttribute('aria-expanded'), 'true', 'click pins the hand');
    await action(page, 'toggle-hand').click();
    assert.equal(await action(page, 'toggle-hand').getAttribute('aria-expanded'), 'false', 'Close dismisses the pinned hand before playing');
    assert.equal(await page.evaluate(() => JSON.stringify(testGame.game_state)), beforePeek, 'viewing and closing the hand consumes no actions');
